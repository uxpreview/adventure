...
| 15 | **The verbs and the law** | **Foundations, and the first session of THE FUN PASS** (`design/THE-FUN-PASS.md`, owner direction 2026-09-01). Touch, carry, sit and throw on one context key; the CHOICE CARD; a scheduled-event clock; the local-rule tier ratified; the five amendments executed in the law files; permission to regress written into `QUALITY-BAR`. Three proofs: a toy on the Common (the well, the cart), a choice card at the toppled king with both doors built, and one scheduled event (the drove at dawn). Ends with the first PLAY SHEET. |
