...
| 15 | **The verbs and the law** ✓ 2026-09-02 | **Foundations, and the first session of THE FUN PASS** (`design/THE-FUN-PASS.md`, owner direction 2026-09-01). **TOUCH, CARRY, SIT and THROW on the one key that looks**, and the prompt says which (`design/specs/controls.md`); **THE CHOICE CARD** in the note card's own system, measured at every width `shoot-mobile` shoots and asserted there; **A SCHEDULED-EVENT CLOCK** (`src/world/events.ts` — *at this hour, in this place, this happens*, a pure function of the hour so it fires whether or not the walker is there, with a `platform`-shaped export); **THE THINGS THE WALKER HAS MOVED** (`src/world/things.ts` — one slot, no inventory, and the border rule written into the file); **A DOOR as a fifth kind of knowledge**. Three proofs: **the well answers a shout and the hay cart can be pushed** (the first two entries in the local-rule register, `QUESTS` §8), **a choice card at the toppled king with both doors built** — put him back and the banners come down, the avenue goes quiet and the moat pool clears — and **the drove moves at dawn** and comes home at dusk. The five amendments executed in `controls.md`, `QUESTS` §2/§3/§8 and `WORLD-SYSTEMS` §6/§7. **And two things found on the way:** every live-coordinate prompt in the game (the rowboat's since Session 6) was nailed to its page-load position by a spread in the POI constructor, and **Joan's wait could never resolve** because nothing in the source taught `fact:the-place-kept`; both closed. `tools/check-verbs.mjs` asserts the law around the verbs. **The first play sheet** (`design/play-sheets/session-15.md`); the play gate handed over, not run. |
| 16 | **The first hour** ✓ 2026-09-03 | The opening the owner chose (`THE-FUN-PASS` §11), built: **you wake in long grass with a bull looking at you**, it charges and never touches you, you run — the run taught by necessity, at the charge — through the gate in the field's hedge, **Nell shuts it behind you**, and the bull stops at the hedge because the hedge is a rule. **The chase runs east to west across the frame**, because a pursuer behind a north-locked camera is never seen. **THE FOUR LURES** from the crossroads — the keep, the mill's smoke, the sea's glint, the city's towers — placed where the frame can hold them and MEASURED (`tools/check-lures.mjs`): desktop holds four at rest, portrait the keep at rest and two more on a peek, never the towers. **The co-walker as a rule of the world** (`src/world/company.ts`, `critique-story-2` MANDATORY 1 closed in the source): the goat follows and stops dead at the Common's edge on every road. **A fence refuses a foot** (`src/world/barriers.ts`), with a stile and a gate that shuts. **DISTRICTS** as a layer under the twelve rects, a card and the map, populated for the Common (the crossroads, the well, the fair ground — new — and the river bend). **NELL's wait built with two doors from the start**, `WAITS_FOR_THE_LINE` to six. The poster kept its ground (`layout.POSTER`) and the walker did not (`SPAWN`). Story gate re-run on Act I: PASSED (`critique-story-3.md`). Art gate on the first minute re-earned on the new frame (`critique-art-10.md`). Play sheet handed over, not run. |
| 17 | **Life** ✓ 2026-09-04 | The four multipliers (`THE-FUN-PASS` §9), all twelve lands at once. **Sixty-three unnamed inhabitants with routines on `events.ts`** (`registerRoutine`, `routineAt`: a list of stops in hour order and the walk between them, a pure function of the hour, every leg a registered event) — a lamplighter four lamps behind and the lamps lit as he reaches them, shutters row by row, a delivery that finds the stall shut, a car that drives to the end of the survey because there is nowhere else a car in this world can go, the nine o'clock from three cars to one door — and the hand-rolled routines (Brim's lamps, the shelter's light, Amos's night walk, Joan's working day) moved onto it. **The animals**, one that reacts in every land: the dog on the Downs' east road as the second co-walker (`company.ts`, four drawings, stops dead at the Downs' edge on both roads out), a herd that parts, a heron that goes up, seals that slip in, pigeons that lift as one, crabs, two cats that wake for a runner, three rooks that cross from the keep to the scarecrow every day and nobody looks up, foxes and bats after dark, and something under the Wide Blue that surfaces once at dusk. **THE WEATHER** (`src/world/weather.ts`): one clock, a pure function of the day and the hour (`clock.day`, saved), so it happens whether or not anybody is out in it — rain as the smudge pass running the drawing, wind as a multiplier on every field's sway that also turns the mill and fills the regatta's sails, fog that closes the vistas and the four lures, a storm at night with lightning and thunder on a crossing; day zero calm at both protected hours, day one authored, the rest hashed. **Night as a different game**: the bull lies down at dusk (one register call), the deep pines go silent after dark, Holt's window is a glow. `?hour=`, `?day=`, `?weather=` on the address bar. `check-fields` drives every routine through its hours; `check-verbs` asserts the dog at both borders, the night's bull, the calm page and the counts; `check-lures` asserts the fog closes all four. Proofs sheet `shoot-session17` (59 framings, both rigs), reviewed in `critique-art-11.md`. Play sheet handed over, not run. |
| 18 | **The roads** ✓ 2026-09-04 | `THE-FUN-PASS` §8, all three fixes. **THE FIFTEEN-SECOND RULE, MEASURED** (`tools/check-roads.mjs`): a tool that walks every road in `layout.ROADS` at 4.1 units a second on both rigs with the clock running, asks at every two seconds what is IN FRAME (the skyline grid projected through the shipping camera, the life registry, the things, the bridges, water beside the road — never an instanced field) and IN EARSHOT (`src/world/earshot.ts`, the placed voices as data, which `App.ts` now reads its distances from; the silences that are places; scheduled events with a place), and fails on fifteen seconds of neither. At noon it found the world more alive than the owner felt — two silences on eleven roads: the canyon trail's run across the pan (22 s, both rigs) and the round's south-west arc on a phone (20 s) — and both got a midpoint (a cairn with a lizard; the cutters' woodpile). **THE ENCOUNTERS**: fifteen of `THE-STRANGERS` Part Three built as routines with a turn, on the clock — a cart with a broken wheel that a wheelwright mends at one and pushes home at three, somebody walking the other way at dusk, a dog that falls in at dawn for half a land and stops dead at a line that is not a border, two people carrying a ladder round the well, a hat going the other way faster than you that stops at the Common's border, a line of combers at low water, a fire lit with nobody at it and two at it by nine and cold ash all the next day, a light in Shelter Cove that is not the mark, a gull on the crest that will not move (a barrier, and the drawing is the gull), a felled pine with the saw in it sawn by three, rings on the tarn at dusk, a funeral up the mill lane at three that stops the land's ambient, a flock on the east road that parts round you and closes behind, the oasis from the wrong direction that is not there, a car on main street with its lights on and its engine off. Twelve already existed; C12 (Wick) is Session 19's, with Wick. **DISTRICTS in all twelve lands** (forty-five, none overlapping, the card and the map already reading them). **THE BICYCLE** (`src/engine/Bicycle.ts`): found on the verge at the mouth of the court, ridden with the one key — GET ON, RING THE BELL on the move, GET OFF stopped — faster than the run and paid back downhill, refusing sand, stairs, water and its border with the walker still on it; the cat and the children answer the bell. **THE PAPER PLANE** (`things.ts` `glide`): a carriable that goes down the air in a line from the hand, off the overlook's lip across the whole cut to the far rim, and is where it landed. **THE 8:15 AS DAILY TRANSIT**: a registered event; the first run is the ending and is written down (`fact:the-8-15-ran`); every morning after it a train stops twelve times for nobody. `check-verbs` section 9 asserts all of it. Proofs sheet `shoot-session18` (36 framings, both rigs). Play sheet handed over, not run. |
| 19 | **The new cast, west and north** ✓ 2026-09-04 | The first local QA pass came first (`design/critiques/qa-local-1.md`): the walker wakes on the gate's row and the bull charges at the shoulder the frame can see, so a due-west run is not a trap (`check-verbs` runs it); **every building, wall, keep, tower, house and hut refuses a foot** (`standee(..., { solid })` writes a `barriers.ts` footprint, clipped off road centrelines; 147 of them) and anything the lens gets inside fades; labels a line-height over their thing, the prompt beside the thing in reach, the region card haloed, the map's district names placed; `shoot-lib` waits for the title instead of fifteen silent seconds. Then the cast. **THE VIKINGS**: a longship on the Holdfast (`longshipTexture` ×3), rowing in at six, beached by seven, in the regatta from noon under a striped sail, back by two — a pure `shipAt(h)` — who roar at anybody on the sand every ten seconds and **cannot land** (eastmost x asserted under −252 all day). A horn on the point they answer from wherever they are. **THE SURFERS** at the Cut: a van, a rack, a wetsuit, two people who check the water at dawn and dusk and never go in; a lit window and the jetty lamp after dark; a board on the wrack that is the coast's first errand. **THE MONSTERS**: the moat red two days in nine with something in it after dark; the-deep every third dusk and no seals on the bar the morning after; a shape at the frame's edge in the deep pines at night for under a second, never twice in seventy seconds. None explained. **WICK, PYE, WREN** drawn, on routines, with waits that are cards with two doors: the fifth banner in Brim's red (learned from the square's cloth) or a bare pole when the king is back, and Wick resting on the verge at dawn (C12); the eighth pot or the pots hauled; the second mark or the fleet finished. `WAITS_FOR_THE_LINE` is seven. **TOYS**: the portcullis, a stone that skips three times off the bar, and the rowboat in the fleet (the racers bear away). Greyweather has stone, a yard and furniture. `check-verbs` §10 on a fresh page; `shoot-session19` (34 framings, both rigs); `critique-art-13` re-earns Greyweather, Longshore and the Wide Blue. Play sheet handed over, not run. |
| 20 | **The new cast, east and south** ✓ 2026-09-05 | `THE-FUN-PASS` §10's last three, the three toys, three second doors — **and the owner's four sketches, drawn as drawn** (`src/world/textures-cast.ts`: a dachshund in a neckerchief, six square sheep, a one-eyed thing in boots, a spotted dog in a bow tie with a paw print). **THE ALIENS**: a ruled pattern burned into the Pale (`scorchDecal`), three of the owner's one-eyed things standing in it by day who turn to a walker and blink when poked (a toy, `alien-blink`), and after dark three lights over the pan (`eyeGlowTexture` hung at roof height) that drift toward a walker and are clamped never inside twelve and never off the pale — stuck too; `pale-hum` in earshot. **THE BARISTA**: a cart on the junction's south-west corner, a routine that calls a name on the hour from seven to six (`the-order-0..11` events, `order-call`) and sets a cup down with it (`cupRowTexture` ×12, a pure function of the hour) — the eleven named people and one blank cup, the second list of names in the world, and the note reads it; **the bow-tie dog** on its own routine, sat beside the cart all day, walking in and out with the barista and stamping the owner's paw on a second `Footprints` (`map` option). **THE DESIGN STUDIO**: a persona board called DENNIS, a journey map, a table, three designers on routines (`designerTexture`), the intercept at eight beside Dennis at the board, and the shelter's glass filling with stickies through the day (`stickyGlassTexture` levels, `stickiesAt(h)` pure); PEEL ONE OFF is the toy. **THE SQUARE FLOCK** in the overflow, one to a bay, scattering square off a walker and re-parking; the long-horned one never moves. **THE LOW DOG** on the green: a companion whose land is the green's district rect, that fetches **THE BALL** (a carriable with `rolls`, so a throw at a run is a kick) and noses it back, and sits at the green's edge when you leave it. **THE BIN** at the junction (E19 righted, then a pushable; into the crossing and the pigeons lift). **THE CHAIR**: a pushable that is a seat with getters and `sit.onSit` — sit, and `things.roll` sends it the way you were facing with the walker riding (`App` follows a seat that moves in both axes). **Three second doors on cards**: the three chairs (`door:the-gap-cut` answers Val's wait now; `door:the-light-off` darkens the street a house a day off a day written into knowledge, `knowledge.first`), the pavement (`door:the-stood-with` / `door:the-walked-round`, three passes → `fact:your-lane`, `yourLaneDecal`; the four seconds do not count while a card is up, `UI.chrome`), the board (`door:the-board-wiped` / `door:the-corner-pressed`; wiped, Dennis skips his eight o'clock). Nine voices; six earshot rows. `check-verbs` §11 on a fresh page; `shoot-session20` (29 framings, both rigs); `critique-art-14`. Play sheet handed over, not run. |
| 21 | **The second door** ✓ 2026-09-05 | `THE-FUN-PASS` §6, finished: **every one of the twelve waits has two doors on a card.** The five that did not — Marget's clock (three doors at the belfry: the bell rings the lamps' hour, or the clock set to eight and her stall never opens, or set to eleven and the lamplighter is three hours early every night), Holt's sea (the trestles a card holding the river: the boat righted, or the sea told and he stops oiling, the marks weather, he is up on the rim), Amos's cistern (the catch a card holding the fold: the lid off, or **a carry** — the can picked up, filled at the oasis, emptied in at the tank in daylight — and he stops carrying and the track grows over), Brack's oar (the tarn a card from outside the twenty: stand at the water, or take the oar, carry it to Hallows and stand it with the eleven, and the twelfth is right and the boat has none and Brack does not turn), Joan's setting (the table a card: sit, which seats you on the door, or clear the second setting and it is not laid again). **THE 8:15 READS THE DOORS** (`Eight15.ts` `onPlatform`): a door that relieves somebody empties their platform whatever else they hold; a door that produced a thing puts the thing there instead — **two cans on the Flats' platform, seven pots on Longshore's** — and the train writes down what it left as it pulls away. **It qualifies on waits DECIDED, either door** (`knowledge.decidedWaits`), because a walker who chose the other way at seven gates has made seven decisions and the ending has to come and read them. **The departure-permanence call, made the smaller way in the owner's absence**: the platforms stay as the doors left them the morning after — the cans and the pots are drawn where they stood, for good, and the people are back where they stand when the doors shut; the larger is written in `PROMPT.md` for the owner. `WAIT_DOORS` in `knowledge.ts`; a card option that `sits`; `things.place` / `things.consume`; five voices. `check-verbs` §12 on two fresh pages; `shoot-session21` (31 framings, both rigs); `shoot-8-15` re-shot with six door-driven framings; `critique-art-15`. Play sheet handed over, not run. |
| 22 | **The story, rewritten** ✓ 2026-09-06 | On top of what is fun, and on top of two things the owner asked for the same day. **THE RAIN, REBUILT** (`Audio.ts` `buildRain`, `rainDrops`): the owner's word for Session 17's rain was *horrible*, and it was a filtered hiss; it is drops now — a seeded buffer of raised-cosine impulses at two densities crossfaded by intensity under a bandpass where paper sits, a low wash under a downpour, and audible drops scheduled half a second ahead — with `check-audio` §9 asserting the crest a hiss cannot have and `render-wavs` writing three rain files. **THE WORN THINGS** (`worn.ts`, `textures-worn.ts`, `Character.wear`): a sixth kind of knowledge, `wear:`, learned by taking a thing off the world at a visible permanent cost to its land — the king's crown (he is bare-headed for good), the hat that runs the coast road (caught at a run or picked up off the border; it never runs again), the spare VISITOR lanyard on the sprint's easel, a helm off the foreshore the morning after the fleet finished (the bow man drawn without it) — one slot on the walker, one lettered HUD button that goes round what is earned and is not on the page until the first is, no screen, nothing counted. **S5 · HOW DEEP built end to end**: Odd on the floor with the line as knowledge, tried in the oasis, paid out on the long water, and a chalk mark cut into the floor below the five with nothing beside it. **`critique-story-2` RECOMMENDED 2 built**: the timetable hands over its fact only once six of the twelve names are held. **THE TONAL RE-KEY**: every note read in its land's key; eight rewritten (the wood road, the tarn's four, riverbend, the mouth, where the road stops, the pale, the car park, the river mouth's prompt) — every one either a medium wink or the funny key in a frightening land. **The story gate, fourth run** (`critique-story-4`): PASSED, NOT WOWED, two recommended, and the reason it is not WOWED is the owner's departure decision. `check-verbs` §13 and §14; six voices. Play sheet handed over, not run. **Not done:** the errands as carry (the twenty remain one line each; the can, the oar and the board are the pattern), the strangers beyond S5. |
| 23 | **Interiors** ✓ 2026-09-06 | The roofless cutaway (`WORLD-SYSTEMS` §11), and the camera first, as §11 said. **A ROOM IS A SECTION** (`src/world/rooms.ts`, `regions/room.ts`, `textures-rooms.ts`): a plan on the ground, an elevation at the north edge, two side walls turned edge-on with the cut face drawn heavy at their south ends, and the house's own front with a door left in its footprint. Walk in and the front goes to a whisper of pencil and the room draws up behind it; walk out and the house is the house, **to the pixel** — a room is born at nothing and comes up only by the walker's blend, which is why no protected framing that looks at a house moved. Four rules, written into the file and asserted: a room OPENS SOUTH, because the camera stands there; a room is INSIDE ITS LAND, so a person drawn in it is in their land; a room is FLAT GROUND to the camera, whose rise term reads zero inside (the loft could not stand on the ramp, slope 0.37, and stands on the plateau just inside the wall); EVERY WALL IS A DRAWING AND A BARRIER. **The camera** (`App.CAM.room`): the rig closes by four units back, one and a half up and most of a unit of aim on desktop, two back in portrait, so the frame's bottom edge lands where the section cuts — and **the rate is the law**: 3.4 units of dolly a second whatever the dolly is, a flat room in a second, the loft under the ridge in four, and `check-verbs` §15 measures the rig every tick on the way in against the walk's 4.1, which is `check-camera`'s ceiling coming the other way. **Three rooms, one per Memory land, three or four things and a person each**: Val's kitchen (the range with the kettle on, a table laid for one with the chair out, the lamp, the hooks with one coat; Val at the range in the middle of the day and by the lamp late, by the window in the dark under her other door), Marget's house — a new front in Brim's back streets — (the dresser, the cloth folded in four on the table at night and the table bare by day, the scale on the wall, level; Marget home for the sixteen hours nobody in the square has ever seen her), and the loft — a new lean-to against the east tower inside Greyweather's wall — (the vat, the stool, a rail with four pegs, three bare, and the fourth bare because the fourth is wet on the bank of the pool; all four hung and the vat rinsed once the king is back). **The errands as carry**, three of the twenty (`THE-STRANGERS` Part Two): E5 the crate, light, and that is the errand; E6 the wet banner, **the first HEAVY thing** (`things.ts`: no run under it, two thirds of a walk, and nothing says so); E18 the small bike, a pushable that leans on its own wall once it is home. **`critique-story-4` RECOMMENDED 1 and 2 built**: THE SHALLOWS in the frightening key, the bow man's helm under the mark door. Three voices; indoors the weather's beds come down. `check-verbs` §15; `shoot-session23` (18 framings, both rigs); `critique-art-16`. Play sheet handed over, not run. **Not built:** the studio, the van, the longship (each an afternoon now), the lit window from inside (the section cuts the wall it is in), seventeen errands. |
| 24 | **The play gate and the juror** | The owner plays the whole thing; the Awwwards pass on the whole build: title, first minute, map, UI feel, mobile portrait, performance audit, then the full-gauntlet critique. |

**AND THEN THE OWNER PLAYED THE WHOLE WORLD, AND THE LADDER WAS RE-CUT
A THIRD TIME** (2026-09-01, after Session 14 merged; the full brief is
`design/THE-FUN-PASS.md` and it is binding). The verdict, in the
owner's words: *"a few areas are good (the forest place) but others
fall flat"*; *"the parts between sections feel empty"*; *"it almost
feels like a chore going from one place to another"*; *"I read things
but that's not fun. I wish I had choices like Fallout, and those
choices did things"*; *"the starting point is bland and expected but
also confuses users"*; *"more regions and activity"*; *"more
interaction and motion"*; *"think of aliens, Vikings, UX designers,
surfers, baristas, monsters, other types of animals"*; and the lens for
all of it: *"serious, emotional, funny, frightening, in the same
game."*

**The diagnosis is one line: the game was built to be read and it has
to be built to be played.** Five laws are amended (`QUALITY-BAR` §3),
a second bar is set for what a land has to PLAY like (`THE-FUN-PASS`
§3), the four categories each own a tone, a cast is added, and the
opening is redesigned from how leading games start. **The story rewrite
waits until the world is fun** (Session 22), because rewriting it first
means rewriting it twice.

**The old Session 15** (the co-walker and the four unbuilt waits) is
split: the co-walker and NELL go to 16, WICK, PYE and WREN to 19. **The
paper plane** goes to 18 and is not deferred again. **Motion & life**
is 17 and is much bigger than it was. **The juror** is last, where it
wants a game whose first hour works. The ordering rule still governs:
the verbs and the choice card change how every land is authored, so
they land first.

**And a third owner's gate is added: THE PLAY GATE.** Every session
from 15 ships a play sheet — a ten-minute script and a build link — and
says in its log that it handed the gate over and did not run it.
