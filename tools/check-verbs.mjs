...
console.log('\nthe second door (Session 21):');
{
  ...
  if (D.amos.filled && D.amos.held === null && D.amos.canState === 'ground' && D.amos.answered && D.amos.decided && D.amos.platform === 'cans') pass('the cistern is filled by hand: the fact is written, the can is on the ground, and THE CANS ARE ON THE FLATS\' PLATFORM instead of him'); else fail(`Amos's door: ${JSON.stringify(D.amos)}`);
