import { stroke, InkBatch, flushBatch, type Ctx2D } from './ink';
...
const UPPER: Record<string, Glyph> = {
  ...
  /* ---- PEN: the rest of the alphabet, so a capital is a capital and
   * not a lowercase letter written tall (a P with a descender is a p) */
  C: cap(0.80,
    [[0.76, -1.22], [0.48, -1.44], [0.16, -1.20], [0.07, -0.70], [0.18, -0.18], [0.50, -0.02], [0.78, -0.22]]),
  J: cap(0.62,
    [[0.20, -1.40], [0.66, -1.42]],
    [[0.48, -1.42], [0.46, -0.30], [0.32, -0.02], [0.10, -0.12], [0.04, -0.36]]),
  K: cap(0.80,
    [[0.13, -1.42], [0.10, -0.02]],
    [[0.70, -1.40], [0.12, -0.62]],
    [[0.30, -0.78], [0.74, -0.02]]),
  O: cap(0.92,
    [[0.46, -1.44], [0.16, -1.22], [0.06, -0.72], [0.18, -0.18], [0.48, -0.02], [0.78, -0.22], [0.86, -0.74], [0.74, -1.24], [0.48, -1.44]]),
  P: cap(0.78,
    [[0.13, -1.42], [0.10, -0.02]],
    [[0.13, -1.42], [0.56, -1.38], [0.70, -1.08], [0.50, -0.72], [0.11, -0.68]]),
  Q: cap(0.92,
    [[0.46, -1.44], [0.16, -1.22], [0.06, -0.72], [0.18, -0.18], [0.48, -0.02], [0.78, -0.22], [0.86, -0.74], [0.74, -1.24], [0.48, -1.44]],
    [[0.54, -0.36], [0.90, 0.08]]),
  S: cap(0.74,
    [[0.70, -1.24], [0.44, -1.44], [0.14, -1.24], [0.22, -0.90], [0.58, -0.66], [0.68, -0.30], [0.40, -0.02], [0.08, -0.18]]),
  U: cap(0.86,
    [[0.10, -1.42], [0.10, -0.40], [0.30, -0.04], [0.60, -0.06], [0.76, -0.40], [0.76, -1.42]]),
  V: cap(0.84,
    [[0.04, -1.42], [0.42, -0.02], [0.80, -1.42]]),
  W: cap(1.12,
    [[0.04, -1.42], [0.28, -0.02], [0.54, -1.00], [0.80, -0.02], [1.04, -1.42]]),
  X: cap(0.80,
    [[0.06, -1.42], [0.74, -0.02]],
    [[0.74, -1.42], [0.06, -0.02]]),
  Z: cap(0.82,
    [[0.04, -1.40], [0.76, -1.42]], [[0.74, -1.42], [0.06, -0.04]], [[0.04, -0.02], [0.80, -0.04]]),
};
...
const DIGIT: Record<string, Glyph> = {
  ...
  // two strokes, like the 5: as one polyline the corner at the top
  // rounded into a bowl and a 4 read as a 9 at chrome sizes (PEN)
  '4': cap(0.78, [[0.60, -1.34], [0.06, -0.44], [0.78, -0.40]], [[0.60, -1.34], [0.58, -0.02]]),
  ...
  // PEN: enough points round each bowl that no turn reads as a corner
  '8': cap(0.78, [[0.40, -0.68], [0.18, -0.86], [0.22, -1.18], [0.42, -1.32], [0.62, -1.18], [0.64, -0.92], [0.42, -0.68], [0.14, -0.46], [0.16, -0.14], [0.40, -0.02], [0.66, -0.14], [0.68, -0.42], [0.40, -0.68]]),
  '9': cap(0.76, [[0.70, -0.86], [0.42, -0.68], [0.16, -0.92], [0.24, -1.22], [0.46, -1.32], [0.66, -1.16], [0.66, -0.50], [0.50, -0.02]]),
};

const PUNCT: Record<string, Glyph> = {
  '.': g(0.30, [[0.13, -0.05], [0.20, 0.02]]),
  ',': g(0.34, [[0.22, -0.06], [0.20, 0.10], [0.08, 0.22]]),
  ...
  /* PEN: the bar of a fraction runs bottom-left to top-right, and the
   * small figures are figures: a 1 with its flag, a 4 with its corner */
  '¼': g(0.86,
    [[0.62, -1.36], [0.16, -0.02]],
    [[0.06, -1.10], [0.18, -1.28], [0.18, -0.70]],
    [[0.74, -0.68], [0.52, -0.34], [0.84, -0.32]], [[0.74, -0.68], [0.74, -0.04]]),
  ...
  /* ---- PEN: what a notebook, a toast, a score and a bubble need ---- */
  '&': g(0.86,
    [[0.74, -0.02], [0.30, -0.50], [0.18, -0.95], [0.32, -1.30], [0.52, -1.22], [0.46, -0.90], [0.10, -0.40], [0.14, -0.10], [0.40, -0.02], [0.62, -0.30], [0.78, -0.44]]),
  '=': g(0.72, [[0.08, -0.66], [0.62, -0.68]], [[0.08, -0.40], [0.62, -0.42]]),
  '%': g(0.82,
    [[0.68, -1.30], [0.08, -0.02]],
    [[0.18, -1.30], [0.08, -1.12], [0.18, -0.94], [0.30, -1.12], [0.18, -1.30]],
    [[0.58, -0.40], [0.48, -0.22], [0.58, -0.04], [0.70, -0.22], [0.58, -0.40]]),
  '#': g(0.76,
    [[0.26, -1.10], [0.16, -0.02]], [[0.56, -1.10], [0.46, -0.02]],
    [[0.06, -0.78], [0.68, -0.82]], [[0.04, -0.34], [0.64, -0.38]]),
  '*': g(0.64, [[0.32, -1.40], [0.30, -0.84]], [[0.08, -1.28], [0.54, -0.96]], [[0.54, -1.28], [0.08, -0.96]]),
  '_': g(0.68, [[0.04, 0.06], [0.62, 0.06]]),
  '‘': g(0.24, [[0.10, -1.34], [0.17, -1.02]]),
  '`': g(0.24, [[0.10, -1.34], [0.17, -1.02]]),
  '@': g(0.98,
    [[0.62, -0.72], [0.48, -0.94], [0.30, -0.86], [0.24, -0.56], [0.36, -0.36], [0.56, -0.46], [0.62, -0.72], [0.60, -0.38], [0.76, -0.34], [0.86, -0.62], [0.72, -1.10], [0.42, -1.26], [0.12, -1.06], [0.04, -0.60], [0.18, -0.14], [0.50, -0.02], [0.76, -0.10]]),
  '<': g(0.64, [[0.56, -1.00], [0.08, -0.54], [0.56, -0.08]]),
  '>': g(0.64, [[0.08, -1.00], [0.56, -0.54], [0.08, -0.08]]),
  '[': g(0.44, [[0.40, -1.42], [0.14, -1.42], [0.14, 0.04], [0.40, 0.04]]),
  ']': g(0.44, [[0.06, -1.42], [0.32, -1.42], [0.32, 0.04], [0.06, 0.04]]),
  '~': g(0.72, [[0.06, -0.50], [0.20, -0.66], [0.36, -0.50], [0.52, -0.34], [0.66, -0.50]]),
  '−': g(0.56, [[0.05, -0.50], [0.50, -0.53]]),
  '°': g(0.40, [[0.20, -1.36], [0.10, -1.24], [0.20, -1.12], [0.30, -1.24], [0.20, -1.36]]),
  '½': g(0.86,
    [[0.62, -1.36], [0.16, -0.02]],
    [[0.06, -1.10], [0.18, -1.28], [0.18, -0.70]],
    [[0.52, -0.56], [0.66, -0.70], [0.82, -0.58], [0.52, -0.06], [0.86, -0.08]]),
  '£': g(0.74,
    [[0.66, -1.24], [0.46, -1.40], [0.26, -1.20], [0.26, -0.20], [0.08, -0.04], [0.70, -0.06]],
    [[0.08, -0.66], [0.52, -0.68]]),
};

/* ---- PEN: what an unknown character draws — a small pencil box, the
 * proofreader's mark for "a glyph goes here". Never a throw, never a
 * gap that swallows a word. */
const BOX: Glyph = { s: [[[0.10, -0.02], [0.10, -0.96], [0.52, -0.96], [0.52, -0.02], [0.10, -0.02]]], adv: 0.62 };
export const BOX_COLOR = '#8a8478';
...
function glyphFor(ch: string): Glyph | null {
  ...
  /* ---- PEN: an accented letter is the letter, written plainly ---- */
  const bare = ch.normalize('NFD')[0];
  if (bare !== ch && bare !== undefined) return glyphFor(bare);
  return null;
}

/** Whether the pen knows this character (the glyph tool asks). */
export function hasGlyph(ch: string): boolean {
  return ch === ' ' || glyphFor(ch) !== null;
}

/** Every character with a skeleton of its own, for the glyph sheet. */
export const GLYPH_CHARS: string =
  Object.keys(LOWER).join('') + Object.keys(UPPER).join('') + Object.keys(DIGIT).join('') + Object.keys(PUNCT).join('');
...
export function writeLine(
  ...
  const base = {
    ...
    passes: o.passes ?? 2,
    smudge,
    /* ---- PEN: a capital's corners are corners (an N is not an n) ---- */
    sharp: 1.7,
  };
  /* ---- PEN: the line's ink is one fill per pass, not one per stroke;
   * the ligatures, the checks and the pencil boxes, which are drawn in
   * another alpha or colour, go straight to the page as before. */
  const batch = new InkBatch(base.passes, o.color ?? INK, base.alpha);
  const inked = { ...base, batch };
  ...
  for (const ch of text) {
    ...
    for (const poly of gl.s) {
      ...
      if (unknown) { stroke(ctx, pts, r, { ...base, color: BOX_COLOR, alpha: 0.5, passes: 1, smudge: false }); continue; }
      stroke(ctx, pts, r, inked);
